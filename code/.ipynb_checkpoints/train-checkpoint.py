...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, scheduler, criterion, epoch, n_labels, train_aug=False):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        targets_x = torch.zeros(batch_size, n_labels).scatter(   # 在第一维度上以target_x原值为下标填充1，
            1, (targets_x.type(torch.int64)).view(-1, 1), 1)     # 把target变成one-hot向量 
        ...
        with torch.no_grad():
            ...
            # For AG News: Chinese: 0.6, ori: 1

            p = (0.6 * torch.softmax(outputs_u, dim=1) + 1 * torch.softmax(outputs_ori, dim=1)) / (1.6)
            # Do a sharpen here.
            pt = p**(1/args.T)
        ...
        if not train_aug:
            all_inputs = torch.cat(                 #将全部有标签的和无标签的输入连接起来
                [inputs_x, inputs_u, inputs_ori], dim=0) #, inputs_ori
                                                        
            all_lengths = torch.cat(
                [inputs_x_length, length_u, length_ori], dim=0)#, length_ori

            all_targets = torch.cat(                #将全部有标签的和无标签的target连接起来
                [targets_x, targets_u, targets_u], dim=0)#, targets_u

        else:
            all_inputs = torch.cat(                
                [inputs_x, inputs_x_aug, inputs_u, inputs_ori, inputs_ori], dim=0)
            all_lengths = torch.cat(
        ...
        Lx, Lu, w, Lu2, w2 = criterion(logits[:batch_size], mixed_target[:batch_size], logits[batch_size:],
                                       mixed_target[batch_size:], None, epoch+batch_idx/args.val_iteration, mixed)
        ...
        if total_steps % 100 == 0:
            ...
            print("epoch {}, step {}, loss {}, Lx {}, Lu {}, Lu2 {}".format(
                epoch, total_steps, loss.item(), Lx.item(), Lu.item(), Lu2.item()))
...
class SemiLoss(object):
    def __call__(self, outputs_x, targets_x, outputs_u, targets_u, outputs_u_2, epoch, mixed=1):
        ...
        if args.mix_method == 0 or args.mix_method == 1:
            ...
            # kl_div=target*(-pred.log()+target.log()), target输入进去自动加log   
